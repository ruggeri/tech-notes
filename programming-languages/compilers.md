- I believe these are notes from Aho Lam Sethi Ullman: Compilers dragon book.
...
- Each "rule" is called a _production_. _Nonterminals_ are sort of
  like types, and they have rules translating them into _terminals_
...
- BTW, a context-free grammar means are rules are like `A -> abc`.
  ...
  - The language is _recognizable_ if a TM exists that will return
  ...
  - Recognizable turns out to be the same as _enumerable_: can a TM
  ...
  - Decidable means there exists a TM to both _accept_ or _reject_.
...
- Given a grammar, we want to transform a string into a _parse
  tree_. This is _parsing_.
  ...
  - I believe this is also called a _concrete syntax tree_.
- Must nail down associativity and precedence of arithmetic operators
  to avoid ambiguity.
- Syntax-Directed Translation. _Syntax-Directed_ means that code
...
- Okay, but how do you parse? _Top down_ parsers start at the root
  nonterminal and try to work their way down to the leaves. They can
  be written efficiently by hand. _Bottom up_ parsers can handle a
...
- You want to be able to do a simple left-to-right scan of the string
  ...
  is called a _predictive parser_.
...
- A _recursive descent parser_ (of which predictive parsers are one
...
- After parsing completes, we often transform the _parse tree_ (or
  _concrete syntax tree_) to an _abstract syntax tree_ (often just
  _syntax tree_).
...
- Symbol table is a table in which we can associate symbols with
  ...
  - Except to deal with scope, it's a _nested_ hash-map.
...
- _Three address code_ is like _static single assignment_. It's an
  intermediate representation (_IR_) used on the way to generating
...
- So NFA construction means that we are joining NFAs together.
  ...
  - (NB: We have elided _parsing_ of the regex, which can be done in
...
- State minimization can reduce redundant states in a DFA. I will
  ...
  - Break down each partition by consider where each state can
    ...
    both transition to new states _in the same partition_.
...
- Two major methods are top-down and bottom-up parsing. Another is
  _universal_ parsing, which can parse any context-free grammar, but
...
- Derivations is a sequence of strings, starting from the start symbol
  ...
  - Anysequence of terminals and nonterminals that can be derived
    from the start symbol is a _sentential form_. If it contains
    only terminals it is a _sentence_.
...
S else S`.
- Left recursion can make parsing hard. You can always _factor_
...
first(\beta) = \empty`.
  - Also, if either `\alpha` or `\beta` can be the empty string,
    ...
    `first(\beta)`.
- To produce a _predictive parsing table_, for every production `A ->
...
- Bottom-up parsing starts from the bottom and works its way
  ...
  backward are called _reductions_.
- Consider a rightmost derivation. Say that `\alphaAw => \alpha\betaw`
  (`w` are terminals). That is, we use the production `A -> \beta` to
  replace `A`. Then, given `\alpha\betaw`, the _handle_ is defined to
  be `\beta`.
- We can obtain a rightmost derivation in reverse by repeatdly _handle
  pruning_. We identify the handle, and then replcae it with the head
  of the production.
- This is the basic idea of _shift-reduce parsing_. Basically, we'll
  scan left to right through the string. We _shift_ characters of
  input, pushing them on a stack. When we can identify a handle, we'll
  _reduce_ it, by replacing it with the left-hand side of the
  production.
...
- The states of an LR parser are sets of _items_, which are
  productions, and our progress through them. For instance, `A -> XY`
  yields three _items_: `A -> *XY`, `A -> X*Y`, and `A -> XY*`. It's
...
- We define `closure(I)`. If `A -> \alpha *B \beta` is in `I`, then we
  ...
  - The _kernel_ of `I` are those items we've matched part of (plus
...
- So the idea is: a closed set `I` is the current state. At the
  ...
  you either _shift_, in which case you use `GOTO(I, a)`, which is a
...
- We can construct a table, called the `SLR(1)` table, which has the
  action to perform for each terminal. It is either a _shift_, to add
  another state, or a _reduce_ to hold onto the terminal a moment, and

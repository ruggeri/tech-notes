...
Quick check: where does the electric field come from? It is created in
...
If $I_\text{diode}$ out of the triode drops but $\Delta V_\text{diode}$
stays constant, we are not in equilibrium. The battery wants to pull
more electrons through the resistor to maintain $IR$. It can do this -
but if $I_\text{resistor} > I_\text{diode}$, then it will build up a
greater charge difference across the cathode/anode of the triode.
...
from the triode. But as you increase the charge difference, $\Delta
V_\text{diode}$ is increasing!

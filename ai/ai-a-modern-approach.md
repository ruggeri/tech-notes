...
* These are all discrete time discussions.
* They talk about first-order Markov processes (variable value at time
  `t` just depends on prior state). There are also second-order et
  cetera.
* Assume a stationary distribution; conditional distribution based on
  prior outcomes stays the same over time.
* They talk about a *sensor model*. Here you don't see the hidden
  state, you see the evidence. The evidence is generated by the state
  at time `t`: `P(e_t | x_t) = P(e_t | x_{0:t}, e{0:t-1})`. That means
  evidence is generated purely on current state.

Tasks in this model include *filtering*, which is estimation of the
current belief state. It's called filtering from the original signal
processing literature. Sometise we call this *state
estimation*. *Prediction* tries to predict some future state, given
evidence up to now. This basically follows from our current belief
state, I think. *Smoothing* is state estimation of *prior* states,
using subsequent evidence too. A *most likely explanation* tries to
figure out the most likely series of states that generated the
observation stream.

The last task is *learning*. Here we don't know the transition
probabilities or the evidence probabilities, and we must learn these
parameters from observed evidence. Typical approach is to do
expectation-maximization.

Now I see why they call this a *Markov sensor model* and not HMM. HMM
is reserved for the *discrete* version. Now, any Markov sensor model
involves forward and backward inference. How this general solution is
executed depends on the form of the transition and evidence
distributions. To do this for HMMs involves some simple matrix math.

Let's discuss the tasks generally, then. Filtering just involves
proceeding step by step and updating our beliefs about the probability
distribution. It should be linear in the number of time steps, and
constant in space. Prediction is basically just a continuation of the
process; it basically just involves the mixing of the probability
distribution. No evidence is generated going forward, so it is
irrelevant.

Smoothing requires forward and backwards information. Basically, you
say, given the probability of the prior evidence, how likely is it you
would be in each state now. But now you say: wait, what about *future*
results? That's the backward part. This is linear time again, and it
is also constant space, because as you go forward you just keep
updating a single forward probability matrix. Likewise for
backward. That's if you want to smooth one time position.

If you want to smooth the whole sequence, it's still linear time, but
you need `O(t)` space. But you knew that, right? Because you are
storing a distribution for each time position.

They make a note: the forward and backward messages can get large if
the distribution does not have a good form. For a discrete
distribution, you know that the messages are just a distribution over
the state space; that is fixed in size. However, for a continuous
distribution, you might have to pass *all the evidence* backward,
which is too much.

Another problem relates to trying to do smoothing online. You don't
want to recompute everything from scratch; you want updates to your
smoothed estimates to be calculated in constant time. Again, depending
on the structure of the transition and evidence distributions, this
may be possible.

To find the most likely sequence, you can't just smooth and pick the
most likely state at each time. You do the Viterbi algorithm. You go
forward, but you change the probabilities passed forward to the
probability `P(X_t, x_{t-1},...x_1 | e_t)`. Here the distribution is
over `X_t`, but it's weighted by the probability of most likely
sequence that got there. You pass these message forward. You need to
track also the most likely sequence for each of those states. That
means you keep `||X||` paths of length `t`.

Now, the point is that for a discrete model which we call a HMM, the
messages are simple and their length is proportional to the number of
states.

Now your problem is going to be if there are way too many states to
keep track of. In the case of continuous variables, this can be
infinite.

(I'm eliding some things because I don't want to get too focused on
this stuff).
